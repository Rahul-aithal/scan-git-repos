...
#         ./scan-git-repos.sh ~/code
#         ./scan-git-repos.sh ~/code | tee repo-status.txt
#
...
for dir in "$CODE_DIR"/*/; do
    ...
    if [ -z "$upstream" ]; then
        NOT_PUSHED+=("$name  [remote exists, but no upstream branch set]")
        continue
    fi
    ...
    if [ "$ahead" -gt 0 ]; then
        NOT_PUSHED+=("$name  [${ahead} commit(s) ahead of remote]")
    elif [ "$behind" -gt 0 ]; then
        NOT_PUSHED+=("$name  [${behind} commit(s) behind — pull needed]")
    else
        PUSHED+=("$name")
    fi
done
